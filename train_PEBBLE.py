        ...
        if self.labeled_feedback > 0:
            ...
            for epoch in range(self.cfg.reward_update):
                if self.cfg.label_margin > 0 or self.cfg.teacher_eps_equal > 0:
                    train_acc = self.reward_model.train_soft_reward(critic=None)
                else:
                    train_acc = self.reward_model.train_reward(critic=None)
    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps:
            if done:
                ...
                #     self.evaluate()
                #evalute below to have script work for variable-length episodes.
